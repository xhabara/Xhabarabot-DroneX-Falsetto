...
function preload() {
  ...
  sound.A = loadSound("RullyShabaraSampleF1.wav");
  sound.B = loadSound("RullyShabaraSampleF2.wav");
  sound.C = loadSound("RullyShabaraSampleF3.wav");
}
...
function setup() {
  ...
  delay1 = new p5.Delay();
  delay1.process(sound.A, 0.12, 0.50, 2300);
  delay2 = new p5.Delay();
  delay2.process(sound.B, 0.12, 0.50, 2300);
  delay3 = new p5.Delay();
  delay3.process(sound.C, 0.12, 0.50, 2300);
...
pvSlider = {
  ...
  delayTimeSlider1 = createSlider(0, 1, 0.50, 0.01);
  delayTimeSlider1.position(20, 10);
  delayTimeSlider2 = createSlider(0, 1, 0.50, 0.01);
  delayTimeSlider2.position(230, 10);
  delayTimeSlider3 = createSlider(0, 1, 0.50, 0.01);
  delayTimeSlider3.position(440, 10);
  
  // add event listeners to delay time sliders
...
function draw() {
  background(
    0,
    0,
    35 + 20 * (1.0 - (pvSlider.A.pos.y + pvSlider.B.pos.y + pvSlider.C.pos.y) / 3)
  );
...
function mousePressed() {
  // play sounds once when mouse is clicked
  for (const [key, audio] of Object.entries(sound)) {
    if (!audio.isPlaying()) {
      audio.setLoop(true);
      audio.play();
